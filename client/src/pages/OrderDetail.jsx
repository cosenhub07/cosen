...
export default function OrderDetail() {
  ...
  const buildWhatsApp = () => {
    ...
    // The "other party" whose number we want to reach
    const targetPhone   = isBuyer ? order.seller?.phone        : order.buyerPhone;
    const targetVerified = isBuyer ? order.seller?.isPhoneVerified : order.buyerPhoneVerified;
    // Current user's phone verification
    const myPhone       = isBuyer ? order.buyerPhone           : order.seller?.phone;
    const myVerified    = isBuyer ? order.buyerPhoneVerified    : order.seller?.isPhoneVerified;

    if (!targetVerified || !myVerified || !targetPhone) return null;

    const serviceTitle = order.service?.title || 'Service';
    const text = encodeURIComponent(`Hi! I ordered your ${serviceTitle} on Cosen 👋`);
    // Normalise phone: strip leading + or spaces
            ...
            <div className="p-4 border-b border-stripe-border bg-slate-50 flex items-center justify-between gap-3 flex-wrap">
              ...
              {waLink ? (
                ...
                <div className="flex items-center gap-1.5 text-xs text-stripe-muted bg-slate-100 px-3 py-1.5 rounded-lg">
                  ...
                  {isBuyer
                    ? 'Verify both phones to unlock WhatsApp'
                    : 'Both must verify phone for WhatsApp'}
                </div>
              )}
            </div>
            ...
            <div className="stripe-card bg-white p-5">
              ...
              {waLink ? (
                <p className="text-xs text-stripe-muted leading-relaxed">
                  Both parties have verified phone numbers. Click the WhatsApp button in the chat header to open a direct conversation with the pre-filled order message.
                </p>
              ) : (
                <p className="text-xs text-stripe-muted leading-relaxed">
                  WhatsApp is unlocked when <strong>both</strong> you and {otherParty.name?.split(' ')[0]} add and verify a phone number in your profile settings.
                </p>
              )}
            </div>
