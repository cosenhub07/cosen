...
export default function OrderDetail() {
  ...
  const buildWhatsApp = () => {
    ...
    // We want to WhatsApp the OTHER party, not ourselves
    const targetPhone = otherParty?.phone;

    // Button works if the other party provided a phone number
    if (!targetPhone) return null;

    const serviceTitle = order.service?.title || 'Service';
    const text = encodeURIComponent(`Hi! I'm messaging regarding "${serviceTitle}" on Cosen 👋`);
            ...
            <div className="p-4 border-b border-stripe-border bg-slate-50 flex items-center justify-between gap-3 flex-wrap shrink-0">
              ...
              {waLink ? (
                ...
                <div className="flex items-center gap-1.5 text-xs text-stripe-muted bg-slate-100 px-3 py-1.5 rounded-lg">
                  ...
                  {isBuyer ? "Seller" : "Buyer"} hasn't provided a phone number yet
                </div>
              )}
            </div>
            ...
            <div className="stripe-card bg-white p-5">
              ...
              {waLink ? (
                <p className="text-xs text-stripe-muted leading-relaxed">
                  {otherParty?.name?.split(' ')[0] || (isBuyer ? "Seller" : "Buyer")} has a phone number on file. Click the WhatsApp button in the chat header to connect directly.
                </p>
              ) : (
                <p className="text-xs text-stripe-muted leading-relaxed">
                  WhatsApp will be available once <strong>{isBuyer ? 'the seller' : 'you'}</strong> provide{isBuyer ? 's' : ''} a phone number in profile settings.
                </p>
              )}
            </div>
            )}
